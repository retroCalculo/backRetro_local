...
#tramo
    geofonos_seleccionados = geofonos_seleccionados.sort_values(by='Absisado')


    num_points = 10


    spline = make_interp_spline(geofonos_seleccionados['Absisado'], geofonos_seleccionados['Zx'], k=3)
    absisado_suavizado = np.linspace(geofonos_seleccionados['Absisado'].min(), geofonos_seleccionados['Absisado'].max(), num_points)
    Zx_suavizado = spline(absisado_suavizado)
    ...
    plt.plot(absisado_suavizado, Zx_suavizado, color='r', label='Suavizado (Splines)', linestyle='--')
